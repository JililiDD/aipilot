...
- **Story 0 (visual direction smoke)** whenever a new page or screen is introduced: ask the user to choose the direction source first — single-file static HTML prototype, generated image prototype, or user-provided prototype. Story 0 creates or records only that visual direction artifact with static/sample data, then stops for confirmation by default; no production flow, API, persistence, or full frontend implementation belongs here. Code is marked `throwaway` by default or `base` only by explicit user choice. Later stories must respect the marking — never silently build production logic on `throwaway` code.

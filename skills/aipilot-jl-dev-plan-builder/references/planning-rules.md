...
- The middle layer has two flavors, identical in structure (Done-when line, AC citations): **User Stories** for user-visible increments; **Task Groups** for non-user-visible work (refactors, pipelines — e.g. "Migrate module A", citing ACs like "behavior unchanged, full suite passes"). Never force user narration onto invisible work. Skip the middle layer entirely when the task list is a handful.
...
- **Story 0 (visual smoke)** whenever a new page or screen is introduced: static-data render first, mandatory user stop, code marked `throwaway` or `base`. Later stories must respect the marking — never silently build production logic on `throwaway` code.

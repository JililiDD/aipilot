...
The CLI is the plugin-vendored `ezreview` 1.0.0 standalone file. It contains the complete runtime and embedded browser assets and is executed directly with Node. Do not substitute `npm`, `npx`, a global `ezreview` command, or any runtime download. The adjacent `LICENSE` and `VERSION` files travel with it.

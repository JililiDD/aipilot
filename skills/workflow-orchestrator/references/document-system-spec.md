...
Status: APPROVED · Version 2.0 · 2026-07-04 (v1.0 frozen 2026-07-02; amended throughout the aipilot redesign — Target Resolution, Story 0 markers, review cadence, dev-builder modes, decisions/lessons split, release markers)
Installed at the documents root as `document-system-spec.md` (master copy ships inside the plugin; the `/aipilot` cold start installs it). It is the single authority on the document system; skills reference these rules instead of restating them. Changes to this spec go through `workflow-evolver` proposals.
...
| `evolution/` | Workflow meta | `signals.jsonl` and `proposals/` — canonical, runtime-neutral location | `workflow-evolver` (any skill may append signals) |
...
**Story → Task hierarchy**: Task is the smallest execution unit and always carries its own verification method. The optional middle layer groups tasks with identical structure in two flavors: User Stories for user-visible increments, Task Groups for non-user-visible work (refactors, pipelines); small task lists need neither. Naming is fixed: **Phase → User Story / Task Group → Task**. Do not introduce "epic" or "ticket".
...
- **File naming**: two tiers only. Ecosystem-convention files keep their conventional all-caps names (`CHANGELOG.md`, `BACKLOG.md`, `AGENTS.md`, `README.md`); **everything else is kebab-case** — state documents (`product-spec.md`, `design-spec.md`, `dev-phase-plan.md`, `agent-guideline.md`), directories (`work-items/`, `design-assets/`, `evolution/`), and all agent-generated files (work-items). Lowercase-only paths eliminate an entire class of agent typos on case-sensitive filesystems.
- **Signal capture**: any skill may append one line to `evolution/signals.jsonl` when it observes a user correction, a misroute, a review failure, a missing gate, or a stale rule — cheap and non-blocking; format and analysis belong to `workflow-evolver`. Capture workflow-rule defects only; product facts, project pits, and implementation changes have their own homes (§2).
...
- **Document root resolution**: every `docs/aipilot/` path in this specification and in all skills means "the resolved documents root". Resolution: a `Documents root:` line under an `## AI Pilot` heading in the project-root `AGENTS.md` → use that path; absent → default `docs/aipilot/` inside the project. `AGENTS.md` is the canonical pointer home in every runtime — skills read it explicitly by path, never via the runtime's auto-loaded context; do not search `CLAUDE.md` or other runtime-specific files for the pointer. The `/aipilot` cold start asks where documents should live (offering to create a project-named subfolder under a user-given directory) and writes the AGENTS.md entry. An out-of-repo root trades away version control — no branch correlation, no clone portability, no git-tracked merge-back history; warn once at cold start, then respect the choice.

- **Interview doctrine**: `references/interview-doctrine.md` governs stance, decision ledger, question format, exit valve, and stall detection for `product-spec-builder` and `design-spec-builder`. Single source in the repo; copies in each skill are synchronized at packaging time, never hand-edited independently.
- **Question format (all skills)**: use the runtime's native structured-question UI when available, otherwise plain chat with lettered options. Recommendations are risk-tiered: mark a recommended option only for low-risk decisions; for anything affecting behavior, data, contracts, scope, or core taste, present trade-offs without a marked recommendation.
- **Evolution data** lives at `docs/aipilot/evolution/` (runtime-neutral). A legacy path may be configured here in `agent-guideline.md`.
- **Timestamps** in CHANGELOG entries: `YYYY-MM-DDTHH:MM:SS` local time.

## 8. Migration Checklist (one-time)

- [ ] **First**: place this specification at the documents root as `document-system-spec.md` (running `/aipilot` once does this automatically). If an existing `agent-guideline.md` contains an older embedded copy, delete that section — project-specific overrides stay.
- [ ] `git mv docs/aidevworkflow docs/aipilot` — the plugin and its document directory are both named `aipilot` now.

- [ ] Rename and lowercase existing documents (git: use `git mv` so case-only renames are tracked): `Product-Spec.md` → `product-spec.md`, `Design-Brief.md` → `design-spec.md`, `Agent-Guideline.md` → `agent-guideline.md`, `DEV-PLAN.md` → `dev-phase-plan.md` (strip its phase detail into phase work-items as they come up — no need to backfill history).
- [ ] Fold any live `IMPLEMENTATION-LOG.md` content into the relevant work-items' Execution Record; archive the file.
- [ ] Append `Product-Spec-CHANGELOG.md` entries into `CHANGELOG.md`; archive the file.
- [ ] `mv .codex/evolution docs/aipilot/evolution` and update the SessionStart hook path.
- [ ] Uninstall the retired `bug-fixer` and `goal-builder` (`goal-creator`) skills if previously installed — their functions live in `dev-builder` (Diagnosis Mode) and `dev-plan-builder` (Goal Wrap).
- [ ] Consolidate any existing `decisions/` ADR files into append-only `decisions.md` (choices) and `lessons.md` (discovered constraints), one dated entry each, oldest first; then remove the directory.
- [ ] Rename `docs/aipilot/prototypes/` → `docs/aipilot/design-assets/`; keep only direction-exploration images and links (prototype code lives in Story 0 branches from now on).
- [ ] Add existing loose work-items' frontmatter; move completed ones to `merged/`.

...
| `design-spec.md` | State | How it looks and interacts | `design-spec-builder` (0-to-1 mode); `workflow-orchestrator` (merge-back of work-item Design sections) |
| `dev-phase-plan.md` | State | Phase map only: order, dependencies, risks, reuse inventory, testing & review strategy. No user stories, no task detail. | `dev-plan-builder` Project Plan Mode; phase pointers backfilled as phases start/merge |
| `memory/decisions.md` | State (append-only) | Choices that bind future work-items AND are not visible in the state documents ("Redis for caching" yes; "this button is red" no — it's in the Design-Spec). Entry heading: `## YYYY-MM-DD <title>`. Superseded by choosing differently — tag the old entry `[superseded by <date entry>]`; never edit past entries. Small by design — every skill reads the whole file when present. | The stage that made the choice: `product-spec-builder`, `design-spec-builder`, `dev-plan-builder`, `dev-builder` |
| `memory/lessons.md` | State (append-only) | Discovered constraints — pits stepped in — that bind future work ("vendor API rate-limits at 10/s", "library X deadlocks under Y"). Entry heading: `## YYYY-MM-DD <title>`. Superseded only when the world changes, and the superseding entry must say what changed; never edited. Small by design — every skill reads the whole file when present. | Whoever discovers, records: `dev-plan-builder`, `dev-builder`. Read-only skills (`code-reviewer`, `release-builder`) note findings; the builder records |
| `memory/agent-guideline.md` | State | Active project-specific workflow conventions and overrides; never plugin-wide defaults | User; `note-keeper` when durable intent is explicit or confirmed |
| `work-items/*.md` | Change | One bounded change, full lifecycle | Four sections, four owners (§3) |
| `work-items/merged/` | Change archive | Merged work-items = the detailed history | `workflow-orchestrator` (move at merge-back) |
| `CHANGELOG.md` | History | One line per merge-back: timestamp, summary, work-item filename (directory derives from status). Plus one `RELEASE <version> — <timestamp>` marker line per confirmed release — the boundary from which the next release's scope derives | `workflow-orchestrator`; `release-builder` (marker line only) |
| `BACKLOG.md` | Pre-requirement | Deferred ideas not yet interviewed. Not requirements until promoted through `product-spec-builder`. | Any stage, with explicit user approval |
| `design-assets/` | Design evidence | Direction-exploration images (e.g. Stitch outputs) referenced by the Design Spec or a work-item Design section; losing directions are kept as anti-reference evidence. Prefer stable share links over committed binaries when the tool provides them. | `design-spec-builder` |

**Memory lifecycle is lazy.** Cold start creates neither `memory/` nor any memory file. A reader treats a missing directory or file as an empty memory category and must not create either merely by reading or starting a session. The skill recording the first entry creates `memory/` and the target file: `# Decisions` plus the first dated entry for `memory/decisions.md`; `# Lessons` plus the first dated entry for `memory/lessons.md`; or the `# Agent Guidelines`, `## Active Workflow Overrides`, and `## Superseded Overrides` structure plus the first rule for `memory/agent-guideline.md`.

**Work-item directories are invariant infrastructure.** Cold start creates `work-items/` and `work-items/merged/`. Every orchestrator startup repairs them if missing because Git does not preserve empty directories. A work-item writer must still ensure `work-items/` exists immediately before creating a work-item, and merge-back must ensure `work-items/merged/` exists immediately before moving a completed item. Missing directories are repaired automatically, never treated as missing project decisions.
...
```markdown
## Quick Overview     <- product-spec-builder / dev-plan-builder: 1-2 sentence Summary, visual flow (ASCII wireframe/pipeline), Before vs After table, optional milestone summary only when it adds distinct value
...
## Execution Record   <- dev-builder (Build/Diagnosis Modes): what was actually done, evidence, reviews, risks
```
...
0. **Quick Overview** — owner `product-spec-builder` (or `dev-plan-builder` on phase-derived items). The human-first executive layer:
   - (1) `> **Summary:**` in 1–2 crisp English sentences.
   - (2) **Visual Workflow / Pipeline**: adaptable to the change scope. For existing changes/refactors, provide a **Before vs After** workflow track comparison; for brand-new features (0-to-1) where no previous flow exists, provide a **single Proposed Workflow Pipeline** without a Before track; for architecture/spikes, provide a layered system/probe board; for trivial one-line fixes, omit the diagram. When a fenced diagram is converted into a visual projection, retain the original source in a closed, explicitly labeled disclosure so the review HTML remains faithful to the Markdown without showing redundant raw code by default.
   - (3) **Overview / Comparison Table**: a Before vs After table for existing modifications, or a Capability/Scope Matrix for new features.
   - (4) **Optional milestone summary**: include only for multi-stage work with meaningful verification dependencies or distinct user-visible releases; omit when it would duplicate the Plan, table, or workflow.
...
4. **Execution Record** — owner `dev-builder` (both its modes), plus `code-reviewer` results recorded by it. Task checklist progress, verification evidence, QA checklist, review outcome, remaining risks.
...
`dev-plan-builder` treats the verification dependency as **the one criterion**; two **detectors** (see its `planning-rules.md`) prompt the search for one but route nothing by themselves:
...
The verdict is **stated, not asked**: planner reports "single work-item / phase map, because X"; the user may override. Phase : work-item = 1:1; phase work-items are created just-in-time by `dev-plan-builder` when the phase is next to build; `dev-phase-plan.md` backfills a pointer and status (planned / in-progress / merged) per phase.

## 5. Execution Granularity

Before implementation starts, `dev-builder` asks one low-risk question — "how far should I go before stopping to report?":
...
Rules: per-task verification happens at every level — granularity changes *reporting stops*, never rigor. The choice is recorded in the Plan section as the default and can be changed mid-run in either direction. Inside `dev-builder`, "continue" authorizes exactly one unit at the current granularity — mirroring the orchestrator's one-stage continue semantics. Reviews are granularity-tied machine gates, not user stops: `code-reviewer` runs automatically at every user story/task group completion (even at whole-work-item granularity), and after every task at per-task granularity; the full pre-merge-back review of the whole work-item is always mandatory on top.

## 6. Merge-back

Performed by `workflow-orchestrator` after `code-reviewer` passes for a work-item, as one uninterrupted bookkeeping step inside the implementation-review loop (no extra user confirmation):
...
4. Ensure `work-items/merged/` exists, set frontmatter `status: merged`, and move the file there.
...
**Self-healing**: at every `workflow-orchestrator` start, before any routing, scan the work-items root for items whose merge-back was interrupted — frontmatter already `status: merged`, or Execution Record shows a passed review with no CHANGELOG entry. Complete the remaining merge-back steps first, then proceed with normal routing.

## 7. Shared Conventions

- **Writing conciseness and high signal-to-noise ratio**: write with high density and zero bureaucratic padding. Scale document depth to the change's actual complexity—simple fixes stay brief, while complex cross-boundary features explain necessary depth. Eliminate cross-section duplication: `Summary` states the core intent; `Existing Context` captures only the necessary background facts and dependencies; `Proposed Behavior` describes the user and system flow using direct imperative phrasing. Structure itemized sections with sequential numbering and concise bold concept anchors for fast human scanning and rich visual card rendering: **Acceptance Criteria** (`- **R-1 (Concept Anchor):** ...` for requirements, `- **D-1 (Concept Anchor):** ...` for design), **Assumptions** (`- **A-1 (Concept Anchor):** ...`), **Open Questions** (`- **Q-1 (Concept Anchor):** [risk: blocks implementation / risks rework / cosmetic] ...`), **Non-Goals** (`- **NG-1 (Concept Anchor):** ...`), and **Edge Cases** (`- **EC-1 (Concept Anchor):** ...`). Preserve existing IDs during revisions, assign the next unused number to a new item, and never renumber an item already referenced downstream. Omit empty template sections when untouched.
- **File naming**: two tiers only. Ecosystem-convention files keep their conventional all-caps names (`CHANGELOG.md`, `BACKLOG.md`, `AGENTS.md`, `README.md`); **everything else is kebab-case** — state documents (`product-spec.md`, `design-spec.md`, `dev-phase-plan.md`, `memory/agent-guideline.md`), directories (`memory/`, `work-items/`, `design-assets/`), and all agent-generated files (work-items). Lowercase-only paths eliminate an entire class of agent typos on case-sensitive filesystems.
- **Project workflow preferences**: lasting project-specific instructions live in `memory/agent-guideline.md`. Explicit durable wording (for example, "from now on", "always", "for this project", or "make this a rule") is approval to write; ambiguous persistence intent requires showing the exact normalized rule and asking whether to persist it. Current-task corrections are not persisted. Conflicting, weakening, or replacement rules require explicit confirmation. Plugin-wide behavior changes belong in the plugin source, never a project document.
...
- **Interview doctrine**: `skills/product-spec-builder/references/interview-doctrine.md` governs the interview discipline for `product-spec-builder` and `design-spec-builder`. Single physical file; `design-spec-builder` reads it via relative path rather than holding its own copy.

...
Status: APPROVED · Version 2.0 · 2026-07-04 (v1.0 frozen 2026-07-02; amended throughout the aipilot redesign — Target Resolution, Story 0 markers, review cadence, aipilot-jl-dev-builder modes, decisions/lessons split, release markers)
Installed at the documents root as `document-system-spec.md` (master copy ships inside the plugin; the `/aipilot` cold start installs it). It is the single authority on the document system; skills reference these rules instead of restating them. Changes to this spec go through `aipilot-jl-workflow-evolver` proposals.
...
| `document-system-spec.md` | State | This constitution — installed from the plugin master at cold start, refreshed on plugin updates | Plugin (via `/aipilot` cold start); amendments via `aipilot-jl-workflow-evolver` |
| `agent-guideline.md` | State | Project-specific conventions and overrides only (e.g. legacy paths); no longer hosts this specification | User; `aipilot-jl-workflow-evolver` Apply Mode |
...
| `evolution/` | Workflow meta | `signals.jsonl` and `proposals/` — canonical, runtime-neutral location | `aipilot-jl-workflow-evolver` (any skill may append signals) |
...
- **Signal capture**: any skill may append one line to `evolution/signals.jsonl` when it observes a user correction, a misroute, a review failure, a missing gate, or a stale rule — cheap and non-blocking; format and analysis belong to `aipilot-jl-workflow-evolver`. Capture workflow-rule defects only; product facts, project pits, and implementation changes have their own homes (§2).
...
- [ ] Uninstall the retired `bug-fixer` and `goal-builder` (`goal-creator`) skills if previously installed — their functions live in `aipilot-jl-dev-builder` (Diagnosis Mode) and `aipilot-jl-dev-plan-builder` (Goal Wrap).

// AI Pilot hook context — POINTERS, not rule copies. The authoritative rules
// live in the documents root (document-system-spec.md) and the
// workflow-orchestrator skill. Keep these short so they never drift.
...
This project may use AI Pilot, a document-driven development workflow. This note only registers that it exists — it does not require using it.

If (and only if) the user wants to build, plan, spec, review, or release something:
1. Read the project-root \`AGENTS.md\` for a \`Documents root:\` line under an \`## AI Pilot\` heading; absent, default to \`docs/aipilot/\` (an explicit file read; do not search \`CLAUDE.md\`).
2. If that root exists, read \`document-system-spec.md\` there and follow the \`workflow-orchestrator\` skill — it, not this note, governs startup, routing, and gates.

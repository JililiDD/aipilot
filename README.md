...
host-specific way to load them. The user-facing name is AIPilot; skill
directories use direct stage names without a plugin prefix.
...
└── skills/
    ...
    ├── note-keeper/                Capture reflex for decisions, lessons, workflow preferences
...
`note-keeper` keeps durable project memory lightweight. Product
...
```bash
node skills/workflow-orchestrator/scripts/render-review.js \
